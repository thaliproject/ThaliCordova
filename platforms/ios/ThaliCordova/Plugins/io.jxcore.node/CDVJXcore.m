...
- (void)pluginInitialize
{
    ...
    NSString * filePath = [[NSBundle mainBundle] pathForResource:@"jxcore_cordova" ofType:@"js"];
    ...
    if (error)
    {
        NSLog(@"Error reading jxcore_cordova.js file: %@", error.localizedDescription);
        assert(0);
    }
...
- (void)Evaluate:(CDVInvokedUrlCommand *)command
{
  ...
  NSString *script = [command.arguments objectAtIndex:0];
    ...
    if (script != nil)
    {
        NSString * scriptWithCallbackId = [NSString stringWithFormat:@"%@, '%@')", script, command.callbackId];
